...
* **Speech-shaped transcript lines:** Whisper cuts its lines arbitrarily (a phrase split between two lines, 10-second run-ons), so every boundary chosen from them was logically ragged. The AI now reads *utterances*: neighbouring segments are joined when there is no real pause between them and the first does not end a sentence.
* **Boundary review (pass 3):** each selected clip is re-examined on its own with 45 s of surrounding transcript. The model first splits the excerpt into scenes (a new scene whenever the subject changes) and then picks the consecutive scenes that form ONE self-contained moment: the payoff plus the setup it needs plus its reaction, never a scene about another topic. This fixes clips without their setup, clips that drag on, and two topics glued into one clip. Back-to-back nominations are no longer merged (only genuinely overlapping ones are). Turn the pass off with `settings.review_boundaries = false` in `config.json` (one extra short request per clip).
* **DeepSeek reasoning mode (on by default):** DeepSeek V4 can think before answering. The boundary review measurably needs it (without it the fast model sometimes picked the wrong scene or stretched a clip to 64 s), so `settings.deepseek_thinking` is `true` by default (also a checkbox in Settings > DeepSeek). The log always states the mode: `DeepSeek reasoning mode: ON (slower, better decisions), 4 requests in parallel`. It is slower (about 6 minutes of AI time for a 90-minute VOD instead of about 2, in parallel batches of 4) and uses more tokens. Turn it off for quick drafts. While the AI works the log shows every request as it starts (`📨 Window 3/8: asking DeepSeek (reasoning) ...`), how long each answer took, and a `⏳ Still waiting ... not frozen` line every 10 seconds; Cancel takes effect immediately instead of after the current batch.
* **Predictable Clip Count:** The LLM only *nominates and ranks* candidates: every 12-minute window must return its N best moments (N follows the density target, with headroom) scored honestly 1-10, weak ones included, because gatekeeping inside a window made the model return too few. The number of exported clips is controlled in code by **Target clips per hour** (Settings, default 12, `0` = no limit) and a minimum score (`min_clip_score`, default 6): the best non-overlapping candidates win. A 1.5-hour VOD yields *up to* about 18 clips (fewer only if there are not enough good candidates) instead of whatever the model felt like.
* **Finishing the thought:** a pause is not the end of an idea (a hesitation in the middle of a sentence is a pause too, and the closing remark of a joke follows the punchline half a second later). The AI sees real silences as `⏸ 1.4s` lines in the transcript (lines without one are continuous speech) and is told to end after the closing remark. In code, `complete_thoughts` then checks both ends against the transcript: a clip that stops inside a sentence is extended to its end (up to 6 s), a short reaction or the opening of the line right after a finished sentence is included, and a clip that starts in the middle of an unfinished sentence begins at that sentence (up to 5 s). A new subject after a real pause is never pulled in.
* **Fast, sliding AI requests:** the AI stages keep several requests in flight (a new one starts the moment another finishes, so one slow answer no longer stalls a whole batch), and the boundary review may add at most 20 s of setup before and 12 s of tail after a clip, which stops it from stretching every clip to about a minute. Clips that turn out to be the same moment after the review are merged instead of being cut into a long clip plus a fragment.
* **Cuts inside pauses, not inside words:** the LLM only proposes a moment; the exact start/end are then placed by the audio (`src/core/cuts.py`). Each boundary is moved to the nearest pause in the waveform (energy envelope, the way VAD-based clippers do it), because ASR timestamps drift by hundreds of milliseconds and a fixed pre/post-roll lands mid-word. On a real 90-minute VOD a blind 0.75 s / 1.25 s pad put ~50% of the cuts inside loud speech. Only real pauses count (>= 0.3 s first, then >= 0.2 s: shorter dips are gaps between syllables, 28% of cuts used to land in them and clipped a word); about 87% of boundaries find one, the rest stay on the transcript's utterance boundary. The exported audio gets a 40 ms fade in and a 150 ms fade out so a cut never sounds chopped. Before that, a coarse step snaps to utterance boundaries (2 s radius), guarantees setup before / reaction after the model-reported `peak_time`, lets a laugh play out and keeps neighbouring clips from overlapping.
...
* `settings.clips_per_hour` (default `12`, `0` = unlimited) and `settings.min_clip_score` (default `6`) in `config.json` control how many clips are exported. `settings.review_boundaries` (default `true`) switches the scene-based boundary review, `settings.deepseek_thinking` (default `true`) DeepSeek's reasoning mode.
